...
"$NSYS" profile --trace=cuda,osrt --sample=none --output /tmp/skein_prof2 --force-overwrite true \
  ./target/release/skein serve --artifact "$ART" --workload cluster/sample_trace.jsonl --cost cluster/cost_constants.toml \
  --prompt "The capital of France is" --max-new-tokens 8 --gpus 0,1
echo "nsys2 exit=$?"
