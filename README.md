...
Skein runs LLMs across multiple GPUs. Give it a model and a cluster — it figures out how to split the work (tensor, pipeline, and expert parallelism), shards the weights, wires up the collectives, and compiles per-device kernels with paged KV cache, CUDA Graphs, and continuous batching.
