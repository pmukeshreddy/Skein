...
/// Sub-f32 floating dtypes whose native C++ operators are ambiguous against the
/// `float` literals and intrinsics the elementwise codegen emits — e.g.
/// `1.0f / v` or `v ? a : b` where `v` is `__nv_bfloat16` compiles to an
/// ambiguous overload under NVRTC (`float`/`bf16` convert both ways). For these
/// we compute in `float` (operands widened, result narrowed on assignment),
/// which is also numerically better. `float`/`int` keep native arithmetic.
fn compute_in_float(dtype: DType) -> bool {
    matches!(
        dtype,
        DType::F8E4M3 | DType::F8E5M2 | DType::F8UE8M0 | DType::Bf16 | DType::F16
    )
}

fn elementwise_value(local: &str, dtype: DType) -> String {
    if compute_in_float(dtype) {
