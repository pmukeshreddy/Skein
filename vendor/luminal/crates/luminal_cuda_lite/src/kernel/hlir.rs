...
extern \"C\" {{
    __global__ void reduce_max_k({dtype} *out, const {dtype} *in{dyn_dims_param}) {{
        // Accumulate the max in fp32 (warp_sums, max_value, block_max all float)
        // and cast to the output dtype only at the final store. Reducing in the
        // element dtype breaks for bf16: the `? :` mixing a bf16 lane value with
        // the float NEG_INF_F is an ambiguous conversion under NVRTC, and bf16
        // reductions are numerically poor anyway (HF does softmax max in fp32).
        ...
        float max_value = NEG_INF_F;
        for (long long i = tid; i < iters; i += THREADS_PER_BLOCK) {{
            max_value = fmaxf(max_value, (float)in[in_start + {iter_stride_of_i}]);
        }}
        ...
        if (warp_id == 0) {{
            ...
            float block_max = tid < cnt ? warp_sums[tid] : NEG_INF_F;
...
impl KernelOp for KernelSumReduce {
    fn compile(
        ...
        let dtype = cuda_dtype(self.dtype);
        let _uses_fp8_storage = matches!(self.dtype, DType::F8E4M3 | DType::F8E5M2 | DType::F8UE8M0);
        // Always accumulate the sum in fp32 (Kahan in fp32), casting to the
        // element dtype only at the final store. A bf16 accumulator both loses
        // precision badly (softmax denominators, RmsNorm) and trips the same
        // bf16/float `? :` ambiguity the max reduction hit under NVRTC.
        let accum_dtype = "float";
        let includes = dtype_includes(&[self.dtype]);
        let n_outputs: Expression = self.out_shape.iter().copied().product();
        let threads_per_block = 256; // 8 warps per block
        let (dyn_defines, _sorted_dims) = generate_dyn_dims_defines(&vars);
        ...
        let iter_stride_of_i = self.iter_stride.to_kernel().replace("const_z", "i");
        // fp32 accumulation: load every element widened to float, seed with a
        // float zero. The final `({dtype})block_sum` store narrows back.
        let load_value = format!("static_cast<float>(in_data[in_start + {iter_stride_of_i}])");
        let zero = "0.0f".to_string();
