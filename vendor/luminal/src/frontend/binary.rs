...
/// Compare two shapes' dims for a binary elementwise op. Falls back to symbolic
/// `simplify()` when structural equality fails, so commutative dynamic dims —
/// e.g. `p + 1` vs `1 + p`, which `concat_along`'s pad-and-add produces for the
/// two operands of a growing cached-decode KV cache — compare equal. The
/// `simplify()` (egglog) path runs only on the rare structural mismatch.
fn dims_match(a: &[Expression], b: &[Expression]) -> bool {
    // `simplify()` (egglog) does not reorder commutative operands, so `p+1` and
    // `1+p` stay structurally distinct. Test semantic equality instead: their
    // difference must simplify to the constant 0.
    a.len() == b.len()
        && a.iter()
            .zip(b)
            .all(|(&x, &y)| x == y || (x - y).simplify().as_num() == Some(0))
}
...
impl Add for GraphTensor {
    ...
    fn add(self, rhs: GraphTensor) -> Self::Output {
        assert!(
            ...
            "Dims must match to add tensors. {:?} vs {:?}",
            self.dims(),
            rhs.dims()
        );
        assert_eq!(
...
impl Rem<GraphTensor> for GraphTensor {
    ...
    fn rem(self, rhs: GraphTensor) -> Self::Output {
        assert!(
            dims_match(&self.dims(), &rhs.dims()),
            "Dims must match to mod tensors. {:?} vs {:?}",
            self.dims(),
            rhs.dims()
        );
        assert_eq!(
...
impl GraphTensor {
    ...
    pub fn lt(self, rhs: GraphTensor) -> GraphTensor {
        assert!(
            dims_match(&self.dims(), &rhs.dims()),
            "Dims must match to lt tensors. {:?} vs {:?}",
            self.dims(),
            rhs.dims()
        );
        assert_eq!(
