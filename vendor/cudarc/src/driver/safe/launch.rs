...
// ---- SKEIN PATCH: scoped capture-safe kernelParams snapshot --------------
//
// `LaunchArgs::arg(&local)` stashes the address of a caller-stack local into
// `self.args`. Upstream `LaunchArgs::launch` passes `self.args.as_mut_ptr()`
// to `cuLaunchKernel`'s `kernelParams`. When stream capture is active, the
// captured graph records that `kernelParams` pointer BY VALUE and re-reads it
// on every replay — but the caller stack (and `self.args`) are gone the
// moment `launch()` returns, so every replay reads garbage from whatever
// happens to be at those addresses now. Symptoms: identical inputs → different
// outputs across runs (uninitialized memory), `argmax`-of-NaN tokens.
//
// Fix: while a stream capture is active on this thread (set/cleared by
// `set_capture_active`), snapshot each arg's first 8 bytes into a thread-local
// append-only pool and pass the pool's stable address as `kernelParams`. The
// pool is never freed during the run. Outside capture the path is byte-for-byte
// identical to upstream (no extra allocation, same `self.args` slice).
//
// SKEIN_CUDARC_PATCH=1 emits one stderr line per capture-window snapshot for
// proof-of-fire diagnostics.
thread_local! {
    static SKEIN_CAPTURE_ACTIVE: std::cell::Cell<bool> = const { std::cell::Cell::new(false) };
    static SKEIN_CAPTURE_POOL: std::cell::RefCell<Vec<(Vec<u64>, Vec<*mut std::ffi::c_void>)>> =
        const { std::cell::RefCell::new(Vec::new()) };
}

/// Set/clear the thread-local "stream capture active" flag. Call with `true`
/// just before `cuStreamBeginCapture` and `false` just after
/// `cuStreamEndCapture` on the same thread.
pub fn set_capture_active(active: bool) {
    SKEIN_CAPTURE_ACTIVE.with(|c| c.set(active));
}

/// Current value of the capture flag.
pub fn capture_active() -> bool {
    SKEIN_CAPTURE_ACTIVE.with(|c| c.get())
}

/// Snapshot every `args[i]`'s first 8 bytes into the thread-local pool and
/// return a `&'static mut [*mut c_void]` slice whose elements point to those
/// stable pool slots. The 8-byte read is safe in practice for the patterns
/// cudarc uses (`&u64`, `&i32`, `&CudaSlice::cu_device_ptr`): pointers are
/// 8 bytes; scalars narrower than 8 bytes live inside stack frames followed
/// by other stack slots, so the over-read is in-bounds, and the kernel reads
/// only the low bytes it expects from each slot (little-endian).
unsafe fn capture_safe_params(
    args: &[*mut std::ffi::c_void],
) -> Option<&'static mut [*mut std::ffi::c_void]> {
    if !capture_active() {
        return None;
    }
    let dbg = std::env::var_os("SKEIN_CUDARC_PATCH").is_some();
    SKEIN_CAPTURE_POOL.with(|pool| {
        let mut pool = pool.borrow_mut();
        let mut vals: Vec<u64> = Vec::with_capacity(args.len());
        for &p in args {
            let v: u64 = unsafe { std::ptr::read_unaligned(p as *const u64) };
            vals.push(v);
        }
        let ptrs: Vec<*mut std::ffi::c_void> = vals
            .iter()
            .map(|v| v as *const u64 as *mut std::ffi::c_void)
            .collect();
        pool.push((vals, ptrs));
        let pool_len_after = pool.len();
        // Re-derive ptrs against the post-move vals heap address, since the
        // Vec move into the pool entry leaves the heap allocation in place
        // but the ptrs we computed pre-move pointed to the same heap so they
        // are still valid — defensively recompute to make the invariant
        // explicit and survive any future Vec layout change.
        let last = pool.last_mut().unwrap();
        let vals_heap_ptr = last.0.as_ptr();
        let n_args = last.1.len();
        for i in 0..n_args {
            last.1[i] = unsafe { vals_heap_ptr.add(i) } as *mut std::ffi::c_void;
        }
        let ptrs_data: *mut *mut std::ffi::c_void = last.1.as_mut_ptr();
        if dbg {
            static FIRED: std::sync::atomic::AtomicUsize =
                std::sync::atomic::AtomicUsize::new(0);
            let n = FIRED.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            if n < 4 || n == 100 || n == 1000 || n % 10000 == 0 {
                eprintln!(
                    "SKEIN_CUDARC_PATCH fired n={} args={} pool_len={} ptrs_addr=0x{:x}",
                    n,
                    n_args,
                    pool_len_after,
                    ptrs_data as usize,
        ...
        // SAFETY: the pool is a thread-local `Vec` that never removes entries
        // during the run, so the inner `Vec`'s heap allocation (which owns the
        // `ptrs` data we point into) lives for the program. We hand out a
        // 'static slice into it for the cuLaunchKernel call.
        let extended: &'static mut [*mut std::ffi::c_void] = unsafe {
            std::slice::from_raw_parts_mut(ptrs_data, n_args)
        };
        Some(extended)
    })
}
...
impl LaunchArgs<'_> {
    ...
    pub unsafe fn launch(
        ...
        let start_event = self
            ...
            .transpose()?;
        // SKEIN PATCH: when stream capture is active on this thread, snapshot
        // the per-arg values into a thread-local stable pool so the captured
        // `kernelParams` pointer remains valid across every replay. When NOT
        // capturing, this returns `None` and the upstream path runs verbatim
        // (no extra allocation, same `&mut self.args` slice passed through).
        let captured = unsafe { capture_safe_params(&self.args) };
        if let Some(stable) = captured {
