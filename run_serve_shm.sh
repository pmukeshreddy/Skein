...
export SKEIN_SHM_ALLREDUCE=1
ART=/tmp/art_big/7923e3a4a6acb152f3856e4bd3ad7d69530433508ec92b04243f002453dc5012
LOG=/tmp/serve_shm.log; : > "$LOG"
for a in $(seq 1 6); do
  timeout 240 ./target/release/skein serve --artifact "$ART" --workload cluster/sample_trace.jsonl --cost cluster/cost_constants.toml --prompt "The capital of France is" --max-new-tokens "${1:-32}" --gpus 0,1 >> "$LOG" 2>&1
  rc=$?
  [ "$rc" -eq 124 ] && { echo "TIMEOUT/HANG"; exit 124; }
